...
def btc_regime_at_time(btc_1h, t, ema_span=50):
    x = btc_1h.loc[btc_1h["ts"] <= t].set_index("ts")["_close"].resample("4h").last().dropna()    
    if len(x) < ema_span:
        return "UNAVAILABLE"
...
def infer_split_boundaries(df):
    """
    Infer the existing calendar boundaries from the observed period labels.

    This does NOT redefine the base periods. It only provides explicit
    timestamps around which Â±7/Â±14 day stress tests can be performed.
    """
    valid = df.loc[df["_eligible"]].copy()

    if "EARLY" not in valid["period"].astype(str).str.upper().unique():
        die("Cannot infer time splits: EARLY period not present.")
    if "MIDDLE" not in valid["period"].astype(str).str.upper().unique():
        die("Cannot infer time splits: MIDDLE period not present.")
    if "LATE" not in valid["period"].astype(str).str.upper().unique():
        die("Cannot infer time splits: LATE period not present.")

    p = valid["period"].astype(str).str.upper()
    e = valid.loc[p.eq("EARLY"), "signal_timestamp"]
    m = valid.loc[p.eq("MIDDLE"), "signal_timestamp"]
    l = valid.loc[p.eq("LATE"), "signal_timestamp"]

    b1 = e.max() + (m.min() - e.max()) / 2
    b2 = m.max() + (l.min() - m.max()) / 2
    return b1, b2
